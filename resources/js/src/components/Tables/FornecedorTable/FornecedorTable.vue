            ...
            <table class="border-separate border-spacing-y-0.5 text-sm w-full max-w-sm mx-auto">
                <thead class="hidden sm:table-header-group">
                    <tr class="select-none">
                        ...
                        <th class="px-4 py-2 text-center">
                            <p class="flex items-center justify-center cursor-pointer" @click="sortTable('marcas')">
                ...
                <tbody>
                    <tr v-for="(item, index) in sortedBody" :key="index" class="tr-class ">
                        ...
                        <td class="td-class text-center" data-label="Name">{{ item.marcas.length }}</td>
                        ...
                        <td class="td-class text-center" data-label="Status">
                            <span @click="this.$emit('update', item)" :class="{
                                ...
                                class="rounded-md px-2 py-1 text-xs font-semibold uppercase antialiased border border-black cursor-pointer">
                                {{ item.status }}
                            </span>
                        </td>
    ...
    computed: {
        sortedBody() {
            return [...this.body].sort((a, b) => {
                ...
                if (this.sortColumn === 'marcas') {
                    // Ordenar por quantidade de marcas
                    return (a.marcas.length - b.marcas.length) * modifier;
                } else {
