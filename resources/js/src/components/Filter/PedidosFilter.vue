<template>
    <div class="flex flex-col gap-2 w-full justify-center items-center whitespace-nowrap">
        ...
        <div class="flex justify-center gap-2">
            ...
            <div class="w-[160px] text-center">
                <h1 class="text-xs">Selecione o Segmento</h1>
                <select v-model="selectedSegmento"
                    class="bg-gray-50 w-[160px] border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5">
                    <option value="">Todas as Opções</option>
                    <option v-for="segmento in uniqueSegmentos" :key="segmento" :value="segmento">{{ segmento }}</option>
                </select>
            </div>
        </div>
...
export default {
    ...
    props: {
        segmentos: {
            type: Array,
            default: () => [] // Garantir que segmentos é sempre um array
        }
    },
    data() {
        return {
            ...
            valorMin: '',
            valorMax: '',
            selectedSegmento: '' // Nova propriedade para o select de segmentos
        };
    },
    computed: {
        uniqueSegmentos() {
            // Verifica se segmentos é um array válido antes de mapear
            if (!Array.isArray(this.segmentos)) {
                return [];
            }

            // A lista de segmentos é simplesmente 'this.segmentos' se já for uma lista de segmentos únicos
            return [...new Set(this.segmentos)];
        },
        isFormValid() {
    ...
    methods: {
        applyFilter() {
            const filters = {
                ...
                segmento: this.selectedSegmento || undefined
            };
            ...
            this.$emit('applyFilter', filteredFilters);
        },
